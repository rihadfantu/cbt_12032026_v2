...
<div class="max-w-7xl mx-auto p-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">{{ $bank->mapel->name ?? '' }} &bull; {{ $bank->guru->name ?? '' }}</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-4">
        <div class="flex border-b border-gray-100">
            ...
            <button onclick="switchTab('import-word')" id="tab-import-word" class="px-5 py-3 text-sm font-medium text-gray-500 hover:text-gray-700">Import Word</button>
        ...
        <div id="content-manual" class="p-4">
            <!-- Soal Grid Navigation -->
            <div class="mb-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-medium text-gray-700">Navigasi Soal</span>
            ...
            <div id="soal-editor" class="hidden">
                <div class="border border-gray-200 rounded-xl p-4">
                    ...
                    <div id="essay-options" class="hidden">
                        ...
                        <div class="mt-2"><label class="text-sm font-medium text-gray-700">Kunci Jawaban:</label>
                        <div id="essay-kunci" contenteditable="true" class="mt-1 border border-gray-200 rounded-lg p-2 text-sm outline-none min-h-[60px]"></div></div>
                    </div>
                    <!-- BS -->
        ...
        <!-- Import Word Tab -->
        <div id="content-import-word" class="hidden p-5">
            <form method="POST" action="{{ route('admin.soal.import-word', $bank->id) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800">
                    <strong>Format Word:</strong> Soal dalam tabel. Baris 1: nomor soal & teks. Baris A-E: opsi. Baris Kunci: jawaban.
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Upload File .docx</label>
                    <input type="file" name="file" accept=".docx,.doc" required class="w-full text-sm border border-gray-300 rounded-lg p-2">
                </div>
        ...
        <div id="content-import-excel" class="hidden p-5">
            <form method="POST" action="{{ route('admin.soal.import-excel', $bank->id) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
...
async function saveAll() {
    ...
    const res = await fetch('{{ route("admin.soal.save", $bank->id) }}', {
        method: 'POST',
...
function insertImg() {
    const url = prompt('URL gambar atau paste Base64:');
...
function switchTab(tab) {
    ['manual','import-word','import-excel'].forEach(t => {
