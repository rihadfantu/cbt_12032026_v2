...
<div class="mb-4 flex gap-2">
    <a href="{{ route('admin.bank-soal.arsip') }}" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium">Arsip Bank Soal</a>
</div>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-blue-50">
                    ...
                    <td class="px-4 py-3 text-right space-x-2">
                        <a href="{{ route('admin.soal.edit', $b->id) }}" class="text-green-600 hover:text-green-800 text-xs font-medium">Edit Soal</a>
                        <button onclick="openEdit({{ $b->id }}, {{ $b->guru_id }}, {{ $b->mapel_id }}, '{{ addslashes($b->title) }}', {{ $b->timer }}, {{ $b->bobot_pg }}, {{ $b->bobot_essai }}, {{ $b->bobot_bs }}, {{ $b->bobot_jodoh }})" class="text-blue-600 text-xs font-medium">Edit</button>
                        <button onclick="arsipkan({{ $b->id }})" class="text-yellow-600 text-xs font-medium">Arsip</button>
                        <form method="POST" action="{{ route('admin.bank-soal.destroy', $b) }}" class="inline" onsubmit="return confirm('Hapus bank soal ini?')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">Belum ada bank soal</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div id="modal-add" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-screen overflow-y-auto">
        ...
        <form method="POST" action="{{ route('admin.bank-soal.store') }}" class="p-5 space-y-4">
            @csrf
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Guru *</label>
                <select name="guru_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    <option value="">Pilih Guru</option>
                    @foreach($gurus as $g) <option value="{{ $g->id }}">{{ $g->name }}</option> @endforeach
                </select>
            </div>
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran *</label>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div><label class="block text-xs font-medium text-gray-700 mb-1">Bobot B/S (%)</label><input type="number" name="bobot_bs" value="0" min="0" max="100" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
                <div><label class="block text-xs font-medium text-gray-700 mb-1">Bobot Jodoh (%)</label><input type="number" name="bobot_jodoh" value="0" min="0" max="100" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
            </div>
...
<div id="modal-edit" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-screen overflow-y-auto">
        ...
        <form method="POST" id="edit-form" class="p-5 space-y-4">
            @csrf @method('PUT')
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Guru *</label>
                <select name="guru_id" id="edit-guru" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none">
                    @foreach($gurus as $g) <option value="{{ $g->id }}">{{ $g->name }}</option> @endforeach
                </select>
            </div>
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran *</label>
                <select name="mapel_id" id="edit-mapel" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none">
                    @foreach($mapels as $m) <option value="{{ $m->id }}">{{ $m->name }}</option> @endforeach
                </select>
            </div>
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Judul *</label><input type="text" name="title" id="edit-title" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Timer (menit) *</label><input type="number" name="timer" id="edit-timer" min="1" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
            <div class="grid grid-cols-2 gap-3">
                <div><label class="block text-xs font-medium text-gray-700 mb-1">Bobot PG (%)</label><input type="number" name="bobot_pg" id="edit-pg" min="0" max="100" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
                <div><label class="block text-xs font-medium text-gray-700 mb-1">Bobot Essay (%)</label><input type="number" name="bobot_essai" id="edit-essai" min="0" max="100" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
                <div><label class="block text-xs font-medium text-gray-700 mb-1">Bobot B/S (%)</label><input type="number" name="bobot_bs" id="edit-bs" min="0" max="100" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
                <div><label class="block text-xs font-medium text-gray-700 mb-1">Bobot Jodoh (%)</label><input type="number" name="bobot_jodoh" id="edit-jodoh" min="0" max="100" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none"></div>
            </div>
            <div class="flex gap-2 justify-end pt-2">
...
<!-- Arsipkan Form -->
<form method="POST" action="{{ route('admin.bank-soal.arsipkan') }}" id="arsip-form" class="hidden">@csrf<input type="hidden" name="id" id="arsip-id"></form>
@endsection
@section('scripts')
<script>
function openEdit(id,guruId,mapelId,title,timer,pg,essai,bs,jodoh){
    ...
    document.getElementById('edit-pg').value=pg;
    document.getElementById('edit-essai').value=essai;
    document.getElementById('edit-bs').value=bs;
    document.getElementById('edit-jodoh').value=jodoh;
    document.getElementById('edit-form').action='/admin/bank-soal/'+id;
    document.getElementById('modal-edit').classList.remove('hidden');
}
function arsipkan(id){ if(!confirm('Arsipkan bank soal ini?')) return; document.getElementById('arsip-id').value=id; document.getElementById('arsip-form').submit(); }
</script>
@endsection
