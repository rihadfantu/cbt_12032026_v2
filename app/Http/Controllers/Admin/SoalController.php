...
class SoalController extends Controller
{
    ...
    {
        $bank = BankSoal::with(['guru', 'mapel'])->findOrFail($id);
        return view('admin.bank-soal.edit-soal', compact('bank'));
    }
                    ...
                    if ($element instanceof \PhpOffice\PhpWord\Element\Table) {
                        ...
                        foreach ($rows as $rowIdx => $row) {
                            ...
                            if ($rowIdx === 0 && count($cells) >= 2) {
                                ...
                                if (in_array($label, ['A','B','C','D','E'])) {
                                    $opsi[$label] = $text;
                                } elseif (strtolower($label) === 'kunci') {
                                    $kunci = strtoupper(trim($text));
                                }
                            }
                        }
                        
                        if ($soalText) {
                            $soals[] = ['tipe' => 'pg', 'soal' => $soalText, 'opsi' => $opsi, 'kunci' => $kunci];
                        }
                    }
                }
            }
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                $soals[] = [
                    'tipe' => 'pg',
                    'soal' => $row[0],
                    'opsi' => ['A' => $row[1] ?? '', 'B' => $row[2] ?? '', 'C' => $row[3] ?? '', 'D' => $row[4] ?? '', 'E' => $row[5] ?? ''],
                    'kunci' => strtoupper($row[6] ?? 'A'),
                ];
            }
